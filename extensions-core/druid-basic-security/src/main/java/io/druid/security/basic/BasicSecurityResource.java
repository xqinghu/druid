...
import io.druid.server.http.security.SecurityResourceFilter;
...
public class BasicSecurityResource
{
  ...
  @ResourceFilters(SecurityResourceFilter.class)
  ...
  )
  {
    ...
    for (String role : roles) {
      Map<String, Object> dbRole = dbConnector.getRole(role);
      if (dbRole == null) {
        return Response.status(Response.Status.BAD_REQUEST)
                       .entity(ImmutableMap.<String, Object>of("error", String.format("role [%s] not found", role)))
                       .build();
      }
    }

    for (String role : roles) {
      dbConnector.assignRole(userName, role);
    }
  ...
  */
  ...
  )
  {
    ...
    if (dbUser == null) {
      return Response.status(Response.Status.BAD_REQUEST)
                     .entity(ImmutableMap.<String, Object>of("error", String.format("user [%s] not found", userName)))
                     .build();
    }
  ...
  public Response updateRole(
      @Context HttpServletRequest req,
      @PathParam("userName") String roleName,
  ...
  )
  {
    ...
    if (dbRole == null) {
      ...
      dbConnector.deleteAllPermissionsFromRole(roleName);
    }
  ...
  */
  ...
  )
  {
    ...
    if (dbRole == null) {
      return Response.status(Response.Status.BAD_REQUEST)
                     .entity(ImmutableMap.<String, Object>of("error", String.format("role [%s] not found", roleName)))
                     .build();
    }
    ...
    if (dbRole == null) {
      return Response.status(Response.Status.BAD_REQUEST)
                     .entity(ImmutableMap.<String, Object>of(
                         "error",
                         String.format("role does not exist: %s", roleName)
                     ))
                     .build();
    }
