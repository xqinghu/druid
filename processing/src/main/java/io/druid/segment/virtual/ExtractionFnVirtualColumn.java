...
public class ExtractionFnVirtualColumn implements VirtualColumn
{
  ...
  {
    final ColumnCapabilities baseColumnCapabilities = factory.getColumnCapabilities(fieldName);
    final ValueType baseColumnType = baseColumnCapabilities != null ? baseColumnCapabilities.getType() : null;

    // Combine our extractionFn + possibly one from dimensionSpec.
    final DimensionSpec extractionDimensionSpec;
    if (dimensionSpec.getExtractionFn() == null) {
      extractionDimensionSpec = new ExtractionDimensionSpec(fieldName, fieldName, ValueType.STRING, extractionFn);
    } else {
      extractionDimensionSpec = new ExtractionDimensionSpec(
          fieldName,
          fieldName,
          ValueType.STRING,
          new CascadeExtractionFn(
              new ExtractionFn[]{extractionFn, dimensionSpec.getExtractionFn()}
          )
      );
    }

    return dimensionSpec.decorate(factory.makeDimensionSelector(extractionDimensionSpec));
  }
