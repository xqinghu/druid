...
public class CliOverlord extends ServerRunnable
{
  ...
  private static class OverlordJettyServerInitializer implements JettyServerInitializer
  {
    ...
    private void authenticatorTest(Injector injector)
    {
      //String json2 = "{\"type\": \"caffeine\"}";
      String json = "{\"type\": \"basic\"}";
      //ObjectMapper mapper = new DefaultObjectMapper();
      ObjectMapper mapper = injector.getInstance(Key.get(ObjectMapper.class, Json.class));
      try {
        //Object y = mapper.readValue(json2, CacheProvider.class);
        Authenticator x = mapper.readValue(json, Authenticator.class);
        log.error("AUTH HEADER: " + x.getAuthChallengeHeader());
        AuthorizationManager y = mapper.readValue(json, AuthorizationManager.class);
        log.error("AM CLASS: " + y.getClass().toString());
      } catch (Exception ex) {
